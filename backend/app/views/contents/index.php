<div class="page-header">
    <div>
        <h2>Nội dung</h2>
        <p>Quản lý bản nháp — duyệt hoặc từ chối trước khi đăng bài</p>
    </div>
</div>
...
<div class="card">
    ...
    <?php if (empty($contents)): ?>
        <div class="empty-state"><p>Chưa có nội dung nào. Vào <a href="<?= url('/products') ?>">Sản phẩm</a> để sinh bản nháp.</p></div>
    <?php else: ?>
                            ...
                            <div class="btn-group">
                                ...
                                <?php if (($content['status'] ?? '') === 'approved'): ?>
                                    <form data-ajax method="POST" action="<?= url('/posts/schedule') ?>">
                                        ...
                                        <button type="submit" class="btn btn-accent btn-sm">Lên lịch</button>
                                    </form>
                                <?php endif; ?>
                            </div>
                        </td>
