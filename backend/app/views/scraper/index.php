...
<div class="grid-12">
    ...
    <div class="card">
        <div class="card-title">➕ Thêm từ khóa cào</div>
        <p class="text-muted text-sm" style="margin-bottom:16px">Hệ thống sẽ tìm sản phẩm theo từ khóa trên sàn TMĐT và lọc theo lượt mua tối thiểu.</p>
        <form data-ajax method="POST" action="<?= url('/scraper/save-config') ?>">
            <div class="form-group">
                <label class="form-label">Từ khóa tìm kiếm</label>
                <input class="form-control" name="keyword" placeholder="vd: laptop gaming, tai nghe bluetooth..." required>
            </div>
            <div class="form-group">
    ...
    <!-- Run Actions -->
    <div class="card">
        <div class="card-title">🚀 Chạy cào dữ liệu</div>
        <p class="text-muted text-sm" style="margin-bottom:16px">Chạy tất cả cấu hình đang hoạt động để cào sản phẩm mới nhất. Quá trình có thể mất vài phút.</p>
        ...
        <?php if (empty($configs)): ?>
            <p class="text-muted text-sm">Chưa có cấu hình nào. Thêm từ khóa ở bên trái.</p>
        <?php else: ?>
            <?php foreach ($configs as $config): ?>
                <div class="scraper-action-row" style="display:flex;align-items:center;gap:8px;margin-bottom:8px;padding:10px 12px;border-radius:8px;background:var(--bg-elevated);">
                    <div style="flex:1;min-width:0">
                        <strong class="text-sm"><?= e($config['keyword']) ?></strong>
                    ...
                    <form data-ajax method="POST" action="<?= url('/scraper/delete-config') ?>" style="margin:0">
                        <input type="hidden" name="config_id" value="<?= (int)$config['id'] ?>">
                        <button type="submit" class="btn btn-sm" style="background:var(--error);color:#fff;" data-confirm="Xóa cấu hình này?">Xóa</button>
                    </form>
                </div>
            <?php endforeach; ?>
        <?php endif; ?>
