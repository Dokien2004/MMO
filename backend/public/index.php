...
$routeModuleMap = [
    ...
    '/links'    => 'LINKS',    '/contents'        => 'CONTENTS',    '/contents/list' => 'CONTENTS',
    '/contents/facebook'=> 'CONTENTS',  '/contents/tiktok'   => 'CONTENTS',
    '/contents/instagram'=> 'CONTENTS', '/contents/threads'  => 'CONTENTS',
...
$getPermissionMap = [
    ...
    '/contents/facebook' => 'contents.view', '/contents/tiktok'   => 'contents.view',
    '/contents/instagram' => 'contents.view', '/contents/threads'  => 'contents.view',
    ...
    try {
        ...
        switch ($channel['channel_type']) {
            case 'facebook_page':
                ...
                $result = $fbPublisher->publish($content, [], $channel);
                break;
            case 'facebook_group':
...
$postPermissionMap = [
    ...
    '/contents/generate-all-facebook'  => 'contents.generate',
    '/contents/generate-images-facebook' => 'contents.generate',
    '/contents/generate-videos-facebook' => 'contents.generate',
    '/contents/generate-all-tiktok'     => 'contents.generate',
    '/contents/generate-images-tiktok'  => 'contents.generate',
    '/contents/generate-videos-tiktok'  => 'contents.generate',
    '/contents/generate-all-instagram'   => 'contents.generate',
    '/contents/generate-images-instagram'=> 'contents.generate',
    '/contents/generate-videos-instagram'=> 'contents.generate',
    '/contents/generate-all-threads'    => 'contents.generate',
    '/contents/generate-images-threads'  => 'contents.generate',
    '/contents/generate-videos-threads'  => 'contents.generate',
    '/contents/approve-facebook'   => 'contents.approve',
    '/contents/approve-tiktok'   => 'contents.approve',
    '/contents/approve-instagram'=> 'contents.approve',
    '/contents/approve-threads'  => 'contents.approve',
    '/contents/generate-image-facebook'  => 'contents.generate',
    '/contents/generate-image-tiktok'    => 'contents.generate',
    '/contents/generate-image-instagram' => 'contents.generate',
    '/contents/generate-image-threads'   => 'contents.generate',
    '/contents/generate-video-facebook'  => 'contents.generate',
    '/contents/generate-video-tiktok'    => 'contents.generate',
    '/contents/generate-video-instagram'=> 'contents.generate',
    '/contents/generate-video-threads'   => 'contents.generate',
    '/contents/regenerate-text-facebook' => 'contents.generate',
    '/contents/regenerate-text-tiktok'   => 'contents.generate',
    '/contents/regenerate-text-instagram'=> 'contents.generate',
    '/contents/regenerate-text-threads'  => 'contents.generate',
    '/contents/regenerate-text'         => 'contents.generate',
    ...
    '/products/save-affiliate'     => 'products.view',
];
...
if ($method === 'POST') {
    ...
    try {
        switch ($path) {
            ...
            case '/products/save-affiliate':
                // Lưu trực tiếp link affiliate vào affiliate_products
                $productId = (int)($_POST['product_id'] ?? 0);
                $affiliateUrl = trim((string)($_POST['affiliate_url'] ?? ''));
                $pdo = db_pdo();
                $stmt = $pdo->prepare("UPDATE affiliate_products SET affiliate_url = ?, updated_at = NOW() WHERE id = ? AND site_id = ?");
                $stmt->execute([$affiliateUrl, $productId, (int)currentSiteId()]);
                if ($stmt->rowCount() === 0) {
                    json_response(false, 'Không tìm thấy sản phẩm #' . $productId);
                }
                json_response(true, 'Đã lưu link affiliate cho sản phẩm #' . $productId);
                break;

            case '/products/select':
                // Cập nhật link affiliate vào user_selected_products
                $productId = (int)($_POST['product_id'] ?? 0);
                $affiliateUrl = trim((string)($_POST['affiliate_url'] ?? ''));
                $pdo = db_pdo();
                $stmt = $pdo->prepare("SELECT * FROM user_selected_products WHERE id = ? AND site_id = ?");
                $stmt->execute([$productId, (int)currentSiteId()]);
                ...
                $data = [
                    ...
                    'price' => $product['price'],
                    'status' => !empty($_POST['status']) ? trim((string)$_POST['status']) : 'pending',
                    'notes' => trim((string)($_POST['notes'] ?? '')),
                ];
                $saved = $userSelectedProductService->upsert($data);
                json_response(true, 'Đã lưu link affiliate.');
                break;
            ...
            // Platform-specific content routes
            case '/contents/generate-all-facebook':
            case '/contents/generate-all-tiktok':
            case '/contents/generate-all-instagram':
            case '/contents/generate-all-threads':
                $platform = substr($path, strlen('/contents/generate-all-'));
                $defaultProvider = $automationSettings['default_content_provider'] ?? 'gemini';
                $providerRaw = trim((string)($_POST['provider'] ?? ''));
                $provider = $providerRaw !== '' ? $providerRaw : $defaultProvider;
                $limit = max(1, min(20, (int)($_POST['limit'] ?? 5)));
                $result = $contentService->generateForEligibleProducts($limit, $provider, $platform);
                json_response(true, 'Da sinh ' . $result['count'] . ' draft content cho ' . strtoupper($platform) . '.');
                break;

            case '/contents/generate-images-facebook':
            case '/contents/generate-images-tiktok':
            case '/contents/generate-images-instagram':
            case '/contents/generate-images-threads':
                $platform = substr($path, strlen('/contents/generate-images-'));
                $limit = max(1, min(20, (int)($_POST['limit'] ?? 5)));
                $result = $imageMediaService->generateForPendingContents($limit, $platform);
                json_response(true, 'Da tao ' . (int)$result['count'] . ' anh cho ' . strtoupper($platform) . '.');
                break;

            case '/contents/generate-videos-facebook':
            case '/contents/generate-videos-tiktok':
            case '/contents/generate-videos-instagram':
            case '/contents/generate-videos-threads':
                $platform = substr($path, strlen('/contents/generate-videos-'));
                $limit = max(1, min(20, (int)($_POST['limit'] ?? 5)));
                $result = $videoMediaService->generateForPendingContents($limit, $platform);
                json_response(true, 'Da tao ' . (int)$result['count'] . ' video cho ' . strtoupper($platform) . '.');
                break;

            case '/contents/generate-image-facebook':
            case '/contents/generate-image-tiktok':
            case '/contents/generate-image-instagram':
            case '/contents/generate-image-threads':
                $contentId = (int)($_POST['content_id'] ?? 0);
                $updated = $imageMediaService->generateForContent($contentId);
                json_response(true, 'Da tao anh AI cho content #' . $contentId . '.', ['data' => ['media_url' => $updated['media_url'] ?? '']]);
                break;

            case '/contents/generate-video-facebook':
            case '/contents/generate-video-tiktok':
            case '/contents/generate-video-instagram':
            case '/contents/generate-video-threads':
                $contentId = (int)($_POST['content_id'] ?? 0);
                $updated = $videoMediaService->generateForContent($contentId);
                json_response(true, 'Da tao video cho content #' . $contentId . '.', ['data' => ['media_url' => $updated['media_url'] ?? '']]);
                break;

            case '/contents/regenerate-text-facebook':
                $contentId = (int)($_POST['content_id'] ?? 0);
                $updated = $contentService->regenerateText($contentId, 'gemini', 'facebook');
                json_response(true, 'Đã tạo lại content text cho content #' . $contentId . '.', ['data' => $updated]);
                break;
            case '/contents/regenerate-text-tiktok':
                $contentId = (int)($_POST['content_id'] ?? 0);
                $updated = $contentService->regenerateText($contentId, 'gemini', 'tiktok');
                json_response(true, 'Đã tạo lại content text cho content #' . $contentId . '.', ['data' => $updated]);
                break;
            case '/contents/regenerate-text-instagram':
                $contentId = (int)($_POST['content_id'] ?? 0);
                $updated = $contentService->regenerateText($contentId, 'gemini', 'instagram');
                json_response(true, 'Đã tạo lại content text cho content #' . $contentId . '.', ['data' => $updated]);
                break;
            case '/contents/regenerate-text-threads':
                $contentId = (int)($_POST['content_id'] ?? 0);
                $updated = $contentService->regenerateText($contentId, 'gemini', 'threads');
                json_response(true, 'Đã tạo lại content text cho content #' . $contentId . '.', ['data' => $updated]);
                break;
            case '/contents/regenerate-text':
                $contentId = (int)($_POST['content_id'] ?? 0);
                $platform = trim((string)($_POST['platform'] ?? 'facebook'));
                $updated = $contentService->regenerateText($contentId, 'gemini', $platform);
                json_response(true, 'Đã tạo lại content text cho content #' . $contentId . '.', ['data' => $updated]);
                break;

            case '/contents/approve-facebook':
            case '/contents/approve-tiktok':
            case '/contents/approve-instagram':
            case '/contents/approve-threads':
                $contentId = (int)($_POST['content_id'] ?? 0);
                $contentService->approveContent($contentId);
                json_response(true, 'Da duyet content #' . $contentId . '.');
                break;
            ...
            case '/posts/schedule':
                ...
                $mediaType = trim((string)($_POST['media_type'] ?? 'auto'));
                $postingService->schedulePost($contentId, $channel !== '' ? $channel : 'fanpage_manual', $scheduledAt, null, $mediaType);
            ...
            case '/posts/schedule-all':
                ...
                $mediaType = trim((string)($_POST['media_type'] ?? 'auto'));
                $result = $postingService->scheduleForApprovedContents($limit, $channel !== '' ? $channel : 'fanpage_manual', $scheduledAt, $intervalMinutes, $mediaType);
            ...
            case '/posts/schedule-selected':
                ...
                $mediaType = trim((string)($_POST['media_type'] ?? 'auto'));
                $result = $postingService->scheduleSelectedContents($contentIds, $channel !== '' ? $channel : 'fanpage_manual', $scheduledAt, $intervalMinutes, $mediaType);
            ...
            case '/posts/update-media-type':
                $postId = (int)($_POST['post_id'] ?? 0);
                $mediaType = trim((string)($_POST['media_type'] ?? 'auto'));
                $postingService->updatePostMediaType($postId, $mediaType);
                json_response(true, 'Đã cập nhật kiểu media cho bài đăng.');
                break;
...
switch ($path) {
    ...
    case '/products':
        ...
        // Build post status per product: which platforms have content for this product
        // generated_contents.product_id references user_selected_products.id (not source_product_id)
        $productIds = array_column($dbProducts, 'id'); // use primary key id
        $sourceProductIds = array_column($dbProducts, 'source_product_id'); // map id => source_product_id
        $idToSourceId = array_combine($productIds, $sourceProductIds);
        $postStatusMap = [];
        if (!empty($productIds)) {
            $placeholders = implode(',', array_fill(0, count($productIds), '?'));
            $postStmt = $pdo->prepare(
                "SELECT gc.product_id,
                        sp.channel,
                        sp.status AS post_status,
                        sp.posted_at
                 FROM generated_contents gc
                 JOIN scheduled_posts sp ON sp.content_id = gc.id
                 WHERE gc.product_id IN ({$placeholders})
                   AND gc.site_id = ?"
            );
            $postStmt->execute(array_merge($productIds, [$siteId]));
            foreach ($postStmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
                $productId = (int)($row['product_id'] ?? 0);
                $sp = (string)($idToSourceId[$productId] ?? '');
                if ($sp === '') continue;
                if (!isset($postStatusMap[$sp])) $postStatusMap[$sp] = [];
                $postStatusMap[$sp][] = [
                    'channel' => $row['channel'] ?? '',
    ...
    case '/contents/list':
        ...
        render('contents/list', [
            'pageTitle'      => 'Duyệt & Lịch',
            'currentPage'    => 'contents_list',
            'contentSummary' => $contentService->summary(),
            'contents'       => $contentService->allContents(),
            'products'       => $userSelectedProductService->all(),
            'automationSettings' => $automationSettings,
    ...
    case '/contents/facebook':
    case '/contents/tiktok':
    case '/contents/instagram':
    case '/contents/threads':
        $automationSettings = $automationSettingsService->get();
        $platformMap = [
            '/contents/facebook'  => 'facebook',
            '/contents/tiktok'    => 'tiktok',
            '/contents/instagram' => 'instagram',
            '/contents/threads'   => 'threads',
        ];
        $platform = $platformMap[$path] ?? 'facebook';
        $platformLabelMap = [
            'facebook' => 'Facebook',
            'tiktok'   => 'TikTok',
            'instagram'=> 'Instagram',
        ...
        render('contents/' . $platform, [
            'pageTitle'    => $platformLabelMap[$platform] ?? 'Content',
            'currentPage'  => 'contents_' . $platform,
            'contentSummary' => $contentService->summary(),
            'contents'    => $contentService->allContents(),
            'products'    => $userSelectedProductService->all(),
            'automationSettings' => $automationSettings,
    ...
    case '/ai-assistant/chat':
        ...
        if (!empty($_POST['product_id'])) {
            $pdo = db_pdo();
            $stmt = $pdo->prepare('SELECT * FROM user_selected_products WHERE id = ? AND site_id = ?');
            $stmt->execute([(int)$_POST['product_id'], (int)currentSiteId()]);
            $product = $stmt->fetch(PDO::FETCH_ASSOC) ?: [];
            if (!empty($product)) {
        ...
        if (!empty($_POST['platform'])) {
            $context['platform'] = $_POST['platform'];
        }
    ...
    case '/ai-assistant/generate':
        requirePermission('contents.view');
        header('Content-Type: application/json');
        $type = trim((string)($_POST['type'] ?? 'copywriting'));
        $productId = (int)($_POST['product_id'] ?? 0);
        $platform = trim((string)($_POST['platform'] ?? 'facebook'));
        if ($productId <= 0) {
        ...
        $assistant = new AiAssistantService();
        $result = $assistant->generateContent($type, $product, $platform);
